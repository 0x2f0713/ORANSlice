...
void nr_update_slice_policy(module_id_t module_id)
{
  ...
  if(json_object_from_file(file) == NULL){
    LOG_W(NR_MAC, "Error in opening rrmPolicy file \n");
	return;
  }

  if(!json_object_object_get_ex(s_obj,"rrmPolicyRatio",&s_array)){
    LOG_W(NR_MAC, "Key rrmPolicyRatio not found in rrmPolicy file \n");
	return;
  }

  if(json_object_get_type(s_array) != json_type_array){
    LOG_W(NR_MAC, "Error in json file array format\n");
	return;
  }
                         ...
                         nfapi_nr_tx_data_request_t *TX_req)
{
  ...
  UE_iterator(UE_info->list, UE) {
    ...
    /* Update the avg dl prbs per frame */
    if (slot == 0 && UE->mac_stats.dl.num_sched_slots > 0){
      UE->avg_prbs_dl= UE->mac_stats.dl.total_rbs / UE->mac_stats.dl.num_sched_slots;
      UE->mac_stats.dl.total_rbs = 0;
      UE->mac_stats.dl.num_sched_slots = 0;
    }
    //SLice_stat
    ...
    if (harq->round != 0) { /* retransmission */
      ...
      if (sched_pdsch->rbSize > 0)
        UE->mac_stats.dl.num_sched_slots += 1;
