...
uint64_t nr_pdcp_module_init(uint64_t _pdcp_optmask, int id)
{
  ...
  if (PDCP_USE_NETLINK) {
    ...
    if(UE_NAS_USE_TUN) {
      ...
      num_if = (get_softmodem_params()->interface_id != 0) ? 1 : num_if;
      netlink_init_tun(ifsuffix_ue, num_if, (get_softmodem_params()->interface_id != 0) ? get_softmodem_params()->interface_id : id);
