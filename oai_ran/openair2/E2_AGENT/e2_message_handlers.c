...
void apply_slicing_ctrl(SlicingControlM* slicing_ctrl_mess)
{
    ...
    uint8_t sum_min_ratio = 0;
    uint8_t sst, min_ratio, max_ratio;
    int sd; // 0x000000 ~ 0xFFFFFF

    sst = slicing_ctrl_mess->sst;
    if (slicing_ctrl_mess->has_sd){
        sd      = slicing_ctrl_mess->sd;
    }
    else{
        sd      = 0xffffff;
    }
    ...
    SL_iterator(SL_info->list, SL) {
      if (SL->nssai.sst == sst && SL->nssai.sd == sd) {
        ...
        LOG_I(E2_AGENT, "Slicing_Ctrl_Msg Applied: NSSAI.SST %d, NSSAI.SD %d, min_ratio %d, max_ratio %d \n", sst, sd, min_ratio, max_ratio);
      }
...
UeListM* get_ue_list(){
    ...
    for(int i = 0; i<num_ues; i++){
        ...
        ue_info_list[i]->has_nssai_sst = 1;
